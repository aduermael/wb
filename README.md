...
Build requirements for source builds and release binaries:

- A Mac with macOS 26.0 or newer
- Xcode 26 or newer, with Swift 6.2 SDKs

Publish requirements for GitHub Releases:
...
Releases are intentionally built from a local Mac for now. There is no GitHub Action because the release binaries need to be built on macOS.

From a clean checkout on `main`, the all-in-one command is:
...
If your Mac does not have `gh`, split the release into two phases. On the Mac:

```bash
./release.sh 0.1.0 --build-only
```

Then copy `dist/v0.1.0/` to an environment with `gh` authenticated to `aduermael/wb`, using the same checkout and commit, and publish:
