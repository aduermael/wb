...
let defaultLintConfiguration = LintConfiguration(
...
enum LintRule: Int, CaseIterable {
	case utf8
	case fileDoc
	case lineEndings
	case finalNewline
...
struct Linter {
	...
	func run(source text: String, relativePath: String = "Test.swift") -> [Violation] {
	...
	private func firstDocLine(in file: SourceFile) -> Int {
		let hasSwiftToolsHeader =
			file.url.lastPathComponent == "Package.swift"
		...
		if lineCount > configuration.maximumLineCount {
			violations.append(
				Violation(
					...
					line: 1,
					message:
						"file has \(lineCount) lines; maximum is \(configuration.maximumLineCount)"
				))
		}
			...
			if line.count > configuration.maximumLineLength {
				violations.append(
					Violation(
						...
						line: lineNumber,
						message:
							"line has \(line.count) characters; maximum is \(configuration.maximumLineLength)"
					))
			}
...
private struct DeclarationScanner {
	...
	func violations() -> [Violation] {
		...
		while index < characters.endIndex {
			if matchesKeyword("func", at: index, in: characters) {
				...
				if let violation = lintCallable(
					kind: "initializer", keywordStart: index, in: characters)
				{
					violations.append(violation)
				}
				index = characters.index(index, offsetBy: 4)
	...
	private func lintCallable(kind: String, keywordStart: Int, in characters: [Character]) -> Violation? {
		guard let openParen = declarationOpenParen(after: keywordStart, in: characters),
			let closeParen = matchingCloseParen(from: openParen, in: characters)
		else {
			return nil
		}
		...
		return Violation(
			...
			message:
				"\(kind) has \(parameterCount) parameters; maximum is \(configuration.maximumFunctionParameters)"
		)
	}
